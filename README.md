...
2. Merge. The shared tag workflow creates an annotated `vX.Y.Z` using `RELEASE_TAG_TOKEN`. Accepting the PR is the gate: it cannot merge until the build is green, so every artifact the release ships already exists.
...
Rehearse before the first tag in a repository: `publish.yml` takes a `workflow_dispatch` with a tag input and its publish steps are gated on a real tag push, so a dispatch verifies the whole chain and publishes nothing. **Re-point the PyPI and crates.io Trusted Publishers to `publish.yml` first** — they match on workflow filename, and a rehearsal cannot catch a stale one because it skips the upload.
