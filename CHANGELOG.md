...
- **Custom labels are lowercase and hyphen-separated**, in `resolve_lanes.py`, `yocto-build.yml` and the docs. `CUDA` and `Yocto` are the exception and keep their existing casing in `runners.json`: GitHub's organisation-wide label registry fixes a label's casing at first creation and the API cannot change it afterwards, so declaring `cuda`/`yocto` there would only make the converger propose the same three no-op removals and additions on every run. Actions label matching is case-insensitive, so the exception costs nothing at runtime.
