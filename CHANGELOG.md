...
- **A lane on an organisation larger runner could fail before it ran anything, with `linker "cc" not found`.** The GitHub-hosted images provide a C compiler; `ubuntu-24.04-arm-xlarge` is a different build and does not. Rust links through `cc` and any dependency with a build script needs one, so every build script in the workspace failed to compile — measured on EdgeFirstAI/ara2-rs run 35382211407, the first repository to put Full on the larger class. `release-wheels.yml` already installed it, which is why the wheel lanes passed on the same image while the test lane died; the guarantee now lives in the `setup-rust` action instead, so all eleven call sites across six workflows get it from one implementation rather than each lane growing its own copy. `require-cc` turns it off. macOS and Windows report and stop, because the toolchain is a property of the image there.
...
- The wheel build's toolchain preflight no longer demands `cc` on Windows. A Windows toolchain is MSVC, which presents as `cl.exe` rather than `cc` and is normally not on `PATH` at all — the `cc` crate locates it through vswhere and the registry — so the check rejected an image that builds perfectly well, on the one platform the `runners` matrix had just been made safe for. Windows is checked for `git` alone; Linux and macOS are unchanged.
