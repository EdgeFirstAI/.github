...
- **`archive-glibc` on `rust-full`, the glibc version the on-target archive is linked against.** The arm lane builds the archive natively, so it required the runner's glibc: 2.39 on Ubuntu 24.04. A board on an older distribution then cannot load a single test binary. Ubuntu 22.04 (JetPack 6) ships 2.35, and every binary fails with `GLIBC_2.39 not found` before any test runs. Setting `archive-glibc: "2.35"` installs a pinned, checksummed zig and uses it as the target's linker, C compiler and C++ compiler. zig ships glibc stubs for any version, which is the mechanism `cargo-zigbuild` uses and the 2.35 floor the runner policy already asks for on anything shipped to a board. `cargo-zigbuild` itself cannot be dropped in, because nextest runs `$CARGO test` and cargo overwrites `CARGO` when it launches a subcommand. C and C++ build scripts go through zig as well: the runner's gcc compiles against 2.39 headers and references symbols such as `__isoc23_strtol` (GLIBC_2.38). The wrapper translates the arguments zig's driver rejects or misreads, following `cargo-zigbuild`'s rules for a gnu target. The `-u __llvm_profile_runtime` that `-C instrument-coverage` passes, which zig reads as an input file, becomes `-Wl,-u,...`. The cc crate's `--target=<rust triple>`, which zig cannot parse, is dropped. Linker flags zig rejects (`--fix-cortex-a53-843419` and others) are dropped. A `-Wl,<file>`, which is how rustc passes a `+whole-archive` native library bundled in an rlib, is passed positionally, as cc and clang do. Empty keeps the native link, so callers that do not set it are unaffected.

### Fixed

- **The full SBOM scans only the tracked files of `source-dirs`, never the working tree.** Given several inputs, scancode scans their common parent, the repository root, and filters afterwards. It first walked everything beneath it: the `venv/` that `sbom-tools` installs scancode into, `target/` and build trees. Its include patterns also match path segments, so `lib`, `include`, `src` and `tests` selected `venv/lib/.../site-packages` and scancode's own license corpus. videostream's full SBOM (`include lib gst src crates videostream tests`) hit the 20-minute timeout on 116 source files. `generate_sbom.sh` now copies `git ls-files` of the source directories into a temporary root and scans that: the same scan finishes in 15 seconds. The long SBOM in repositories whose `source-dirs` include `src` or `tests` has the same cause.

- **`board-run` names each profraw after the board's hostname as well as the process, so coverage from several boards no longer collides.** The pattern was `board-%p-%m.profraw`. With more than one board in a lane, two boards can hand the same PID to processes of the same binary, which produces the same file name, and the merge job downloads every board's profraw into one directory with `merge-multiple`. One file silently replaced the other and that process's coverage was lost, with nothing failing. The pattern is now `board-%h-%p-%m.profraw`, which LLVM expands with the host name, so names are unique across boards without the workflow having to know which runner it is on.
