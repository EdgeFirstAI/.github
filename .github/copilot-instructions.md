...
- `rust-full`: `lanes` (comma-separated set of `host`, `hardware`, `gpu`; `all` means `host,hardware`), `boards`, `nightly`, `runner-class-linux` / `-linux-arm` / `-macos` / `-windows`, `gpu-args` (opt into the CUDA lane; non-empty is what enables it), `gpu-pre-command` (GPU job setup; falls back to `pre-command`), `env` (KEY=VALUE lines, every lane), `pre-command` (host jobs), `board-pre-command` (board; falls back to `pre-command`), `board-extra-args` (`-j 1` and similar; not used for archive), `archive-args` (nextest archive features/packages; empty uses `nextest-args`), `archive-glibc` (glibc version the board archive links against, e.g. `2.35`; empty uses the runner's 2.39, which a board on an older distribution cannot load). The SonarCloud job is skipped automatically when the repository has no `sonar-project.properties`.
